(() => {
    ...
    function parsePuntosRollos(record) {
        ...
        for (let index = 0; index < total; index += 1) {
            ...
            if (defecto || cantidad) {
                card.defectos.push({ defecto, cantidad });
            }
        }
    ...
    function buildRolloFieldHtml(field, label, value, isMedida) {
        return `
            <label class="rollo-field">
                <span class="rollo-field-label">${label}</span>
                <input
                    class="puntos-input${isMedida ? ' puntos-input-medida' : ''}"
                    type="text"
                    inputmode="numeric"
    ...
    function buildDefectoLineHtml(item, defectoIndex) {
        ...
        return `
            <div class="rollo-defecto-row" data-defecto-index="${defectoIndex}">
                ...
                <button type="button" class="puntos-row-remove" data-action="remove-defecto" aria-label="Quitar defecto">&times;</button>
            </div>
        `;
    }

    // Una tarjeta por rollo: arriba sus datos, adentro la lista de defectos.
    function buildRolloCardHtml(card, rolloIndex) {
        const defectos = Array.isArray(card.defectos) ? card.defectos : [];
        ...
        return `
            <div class="rollo-card" data-rollo-index="${rolloIndex}">
                <div class="rollo-card-head">
                    ${buildRolloFieldHtml('rollo', '#Rollo', card.rollo, false)}
                    ${buildRolloFieldHtml('ancho', 'Ancho', card.ancho, true)}
                    ${buildRolloFieldHtml('densidad', 'Dens', card.densidad, true)}
        ...
        return `
            <div class="puntos-block" data-record-id="${TintoreriaUtils.escapeHtml(recordId)}">
                ...
                <div class="rollo-card-list">
                    ${entry.rollos.map((card, rolloIndex) => buildRolloCardHtml(card, rolloIndex)).join('')}
                </div>
    ...
    function renderPuntos(force = false) {
        ...
        const renderKey = buildPuntosRenderKey(selectedRecords);
        if (!force && renderKey === state.lastPuntosRenderKey) {
            return;
        }

        state.lastPuntosRenderKey = renderKey;
        els.puntosContainer.innerHTML = selectedRecords.map(buildPuntosBlockHtml).join('');
    }
    ...
    function handlePuntosInput(event) {
        ...
        if (field === 'cantidad') {
            ...
            context.rollo.defectos[context.defectoIndex].cantidad = target.value;
            context.entry.dirty = true;
            return;
        }

        if (field !== 'rollo' && field !== 'ancho' && field !== 'densidad') {
            return;
        }
    ...
    function renderDefectoList() {
        ...
        const clearHtml = currentValue
            ? '<button type="button" class="defecto-option defecto-option-clear" data-defecto-value="">Quitar defecto</button>'
            : '';
    ...
    function findRolloCardElement(recordId, rolloIndex) {
        ...
        const block = Array.from(els.puntosContainer.querySelectorAll('.puntos-block'))
            .find((candidate) => candidate.getAttribute('data-record-id') === String(recordId || ''));
    ...
    function handlePuntosClick(event) {
        ...
        if (action === 'remove-defecto') {
            const context = findDefectoContext(actionButton);
            if (!context) {
                return;
            }

            // Quitar el ultimo defecto no borra el rollo: para eso esta la X
            // de la cabecera de la tarjeta.
            context.rollo.defectos.splice(context.defectoIndex, 1);
            entry.dirty = true;
            renderPuntos(true);
        }
    }
